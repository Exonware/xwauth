...
**Audience:** Operators integrating **SAML 2.0** IdPs with the **XW** stack (`exonware-xwauth` + `exonware-xwlogin` HTTP mixins + `xwauth-api` routes).  
...
| Goal | Command / wiring |
|------|------------------|
| **SAML dependencies** | `pip install "exonware-xwauth[saml]"` (`signxml`, `lxml`) — see [REF_39_EDITION_AND_SKUS.md](REF_39_EDITION_AND_SKUS.md). |
| **Full self-hosted AS embedding** | `pip install "exonware-xwauth[enterprise]"` (SAML + storage + login handlers). |
...
**HTTP surfaces (reference server):** SAML metadata, ACS, and discovery are registered under **`/v1/auth/sso/...`** via `xwauth-api` `AUTH_SERVICES` (e.g. `.../sso/saml/metadata`, `.../sso/saml/acs`, `.../sso/discovery`). Route family **`saml`** for ops headers / SLIs ([REF_62](REF_62_OPS_SLI_REGISTRY_V1.md)).
