...
**Status:** Baseline seed for Phase 0/1; **CI parity** with `xwauth-connector-api` (see §5)

---

## 1) Purpose

Provide a stable endpoint-family to SLI map that can be populated from `xwauth-connector-api` route registration and used by dashboards, alerting, and scorecards.
...
**CI:** `exonware-xwauth-connector-api` runs `tests/0.core/test_sli_registry_parity.py`; every `route_family` produced for `AUTH_SERVICES` must appear in this table. Canonical set in code: `exonware.xwauth.connector_api.sli_catalog.SLI_REGISTRY_V1_ROUTE_FAMILIES`.
...
- **Enforced:** `xwauth-connector-api` workflow **core-tests** runs `pytest tests/0.core -m xwauth_connector_api_core` (includes `test_sli_registry_parity.py`).
