...
|---------------|------------|
| **Tenant / custom domain** | Public **issuer** URL + TLS at ingress (see xwauth-connector-api Helm / [REF_33](REF_33_PARTNER_INTEGRATION_MATRIX.md)). |
...
1. Compare **access token** TTL and **refresh** behavior; set `access_token_lifetime` / env overrides on xwauth-connector-api.  
