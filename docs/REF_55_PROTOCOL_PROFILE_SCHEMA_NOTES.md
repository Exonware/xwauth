...
**Audience:** `xwauth` and `xwauth-connector-api` maintainers
...
`xwauth-connector-api` startup enforces profile rigor in two stages:
...
`xwauth-connector-api` publishes protocol scorecard artifacts through CI:

- Workflow: `xwauth-connector-api/.github/workflows/protocol-conformance.yml`
- Generator: `xwauth-connector-api/scripts/protocol_scorecard.py`
...
- Confirm `xwauth-connector-api` workflow **Protocol Conformance Gate** passed for the same commit on all matrix profiles (`A`, `B`, `C`).
