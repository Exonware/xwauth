...
**Project:** `xwauth`  
**Scope:** `xwauth` core with explicit integration anchors in `xwauth-api` and `xwsystem`  
...
- **`xwauth-api` (transport):** endpoint surface, middleware, telemetry propagation, startup profile loading.
...
- `xwauth-api/src/exonware/xwauth_api/services/__init__.py` (route registry)
- `xwauth-api/src/exonware/xwauth_api/server.py` (startup wiring)
- `xwauth/src/exonware/xwauth/handlers/mixins/system.py` (`/health`, `/metrics`)
- `xwauth/src/exonware/xwauth/handlers/mixins/auth_core.py` (critical auth paths)
- `xwauth/src/exonware/xwauth/sessions/manager.py` and `security.py` (session lifecycle)
...
- Load tier/runtime profile at startup in `xwauth-api`.
...
- `xwauth-api` loads ops tier/profile from env at startup (`XWAUTH_OPS_TIER`, `XWAUTH_OPS_PROFILE_PATH`) and stores resolved state on `app.state`.
- `xwauth-api` installs correlation middleware emitting:
  - `X-Request-Id`
  - `X-Correlation-Id`
  - `X-Trace-Id`
  - `X-Route-Family`
