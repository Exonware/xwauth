...
Define a migration-safe strategy for persisted auth artifacts so `xwauth` can evolve without breaking existing data in `xwstorage.connect`-backed environments.
...
- `xwstorage.connect` owns persistence mechanics only.
