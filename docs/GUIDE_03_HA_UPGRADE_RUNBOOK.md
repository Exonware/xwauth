# GUIDE_03 â€” HA, backup, and upgrade runbook (auth stack)

**Audience:** Operators of **xwauth** + **xwlogin** + **xwauth-connector-api** (reference server or embedded FastAPI).  
...
|-------|------------------|--------|
| **Auth persistence** | `XWAUTH_API_STORAGE_PATH` (JSON / xwstorage.connect-backed dir) | Codes, refresh handles, sessions â€” **treat as confidential**. |
...
### 2a. File-backed JSON storage (default xwstorage.connect path)

When `XWAUTH_API_STORAGE_PATH` (or your appâ€™s `base_path`) points at a **directory** of JSON files:
...
For **PVC** backups in Kubernetes, snapshot the volume or use your storage classâ€™s backup job; see **xwauth-connector-api** [deploy/k8s/README.md](https://github.com/exonware/xwauth.connector-connector-api/blob/main/deploy/k8s/README.md).
...
4. **Migrate** storage only if the release documents a migration (otherwise file-backed state is usually forward-compatible within minor versions â€” verify).  
...
- Plan **overlap** only if you introduce **asymmetric** signing (JWKS) or dual-secret validation â€” not the default HS256-only path.  
...
- Reference server golden path: **exonware-xwauth-connector-api** package / repo â€” `docs/GOLDEN_PATH_DEPLOY.md`.  
