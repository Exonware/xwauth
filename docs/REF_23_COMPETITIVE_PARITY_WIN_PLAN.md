...
- **`xwauth-api` (network surface over `xwapi`):**
...
- **`xwauth-web` (admin/end-user UX on `xwui-base`):**
  ...
  - Must consume `xwauth`/`xwauth-api` contracts, not fork policy logic.
...
|---|---:|---:|---:|---|---|
| OAuth2/OIDC | `hydra-master` | 3.4 | 5.0 | `xwauth` + `xwauth-api` | M1-M2 |
...
| Authorization | `keycloak-main` | 2.9 | 5.0 | `xwauth` + `xwauth-api` | M2-M3 |
| Multi-tenant | `zitadel-main` | 2.5 | 5.0 | `xwauth` + `xwauth-api` + `xwauth-web` | M2-M4 |
| Observability | `keycloak-main` | 2.4 | 5.0 | `xwauth` + `xwauth-api` | M2-M4 |
| Quality gates | `keycloak-main` | 2.7 | 5.0 | cross-package | M1-M4 |
| DX/docs | `logto-master` | 3.0 | 5.0 | `xwauth-api` + `xwauth-web` | M3-M4 |
...
### Track B — Transport parity (`xwauth-api`)
