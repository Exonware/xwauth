...
**Project:** `xwauth` ecosystem (`xwauth`, `xwlogin`, `xwauth-connector-api`)  
...
| 6 | Reproducible performance benchmark harness + published results | Done* | Ops credibility | *Scope:* library microbench (`exonware.xwauth.connector.bench`) + **HTTP TestClient** bench (`exonware.xwauth.connector_api.bench`) + unit tests + [xwauth benchmarks/README.md](../benchmarks/README.md) + xwauth-connector-api `benchmarks/README.md` + workflow `http-bench.yml`. *Optional:* socket load (`oha`/`wrk`) + checked-in result JSON for TCO. |
| 7 | Full admin parity: every console action has an API + OpenAPI | Done* | Keycloak-class ops | *`ops/admin_api_openapi_parity.py`, `test_admin_api_openapi_parity.py`, [REF_42_ADMIN_API_OPENAPI_PARITY.md](REF_42_ADMIN_API_OPENAPI_PARITY.md); route inventory + 100% parity in `xwauth-connector-api` still required* |
...
| 16 | Email / magic-link operational playbook (SPF/DKIM/DMARC, bounces) | Done* | Reliability | *`exonware.xwauth.connector.ops.email_magic_link_ops`, tests `ops_tests/test_email_magic_link_ops.py`, [REF_28_EMAIL_MAGIC_LINK_OPS.md](REF_28_EMAIL_MAGIC_LINK_OPS.md) |
...
- **Benchmark harness (REF_25 #6):** `exonware.xwauth.connector.bench` (JWT microbench) + `exonware.xwauth.connector_api.bench` (token `TestClient` loop) + `xwauth-connector-api/scripts/http_bench.py` (monorepo path bootstrap); tests under each repo’s `tests/1.unit/bench_tests/`; CI: `xwauth-connector-api/.github/workflows/http-bench.yml`.
...
|-----------|--------------------------------------|--------------------|--------------------------------|
| Moving parts | `xwauth` + `xwlogin` + `xwauth-connector-api` + storage + your app | Single product, heavy JVM process | Multiple services + custom login |
...
1. **Semantic versioning** for `exonware-xwauth-connector`, `exonware-xwauth-identity`, `exonware-xwauth-connector-api`: MAJOR for incompatible public API or wire-format changes; MINOR for backward-compatible features; PATCH for fixes.
