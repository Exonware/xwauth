...
**Audience:** Operators and integrators exposing **SCIM 2.0** (`/v1/scim/v2/*`) from **xwauth** + **xwauth-api** to IdPs (Entra provisioning, Okta, etc.).  
...
| Area | Location |
|------|-----------|
| **HTTP routes** | `xwauth-api` `AUTH_SERVICES` — `GET/POST .../scim/v2/Users`, `Groups`, `ServiceProviderConfig`, etc. (`route_family`: **`scim`** — [REF_62](REF_62_OPS_SLI_REGISTRY_V1.md)). |
| **Handlers** | `src/exonware/xwauth/handlers/mixins/scim.py` — auth, paging query parsing, `If-Match`, org query guard (`org_id` vs token org). |
| **Service semantics** | `src/exonware/xwauth/scim/service.py` — `list_response` (`startIndex`, `count`, `filter`), create/patch/delete. |
| **Filter / patch** | `scim/filtering.py`, `scim/patch.py` |
| **Tests** | `xwauth/tests/1.unit/scim_tests/`, `xwauth-api/tests/2.integration/test_scim_policy_api.py` |
...
| **`count`** | **`0`** is valid (metadata-only list in some profiles); negative values are invalid. Malformed query integers fall back to defaults in HTTP layer — **confirm** your product policy: strict `400` vs safe defaults (`src/exonware/xwauth/handlers/mixins/scim.py` uses `try/except` → `1` / `100`). |
