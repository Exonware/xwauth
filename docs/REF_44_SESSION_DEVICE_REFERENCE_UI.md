...
**Purpose:** REF_25 #14 — end-user **“my sessions” / “my devices”** in the reference stack (`xwlogin` + `xwauth-connector-api`).  
**Checklist API:** `exonware.xwauth.connector.ops.session_device_reference_ui.session_device_reference_ui_checklist()`  
...
Bearer-authenticated **session management** mixins live in the library (wire via your FastAPI app / xwauth-connector-api):
...
Source: `src/exonware/xwauth.connector/handlers/mixins/sessions.py`.

## Integration tests (xwauth-connector-api)

Bearer-authenticated list/revoke behavior is covered in `xwauth-connector-api/tests/2.integration/test_api_services.py` (`TestAuthSessionsWithBearer`; uses password grant enabled only in that fixture for test isolation).
