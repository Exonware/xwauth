...
1. **Fork** [xwauth-api](../../xwauth-api/) Helm chart values for your cluster naming and ingress.  
...
4. Document **one golden path** for engineering: same as [GOLDEN_PATH_DEPLOY.md](../../xwauth-api/docs/GOLDEN_PATH_DEPLOY.md) but with your hostname and Secret naming.

---

## 4. Terraform stub (namespace + JWT Secret + Helm)

A minimal **IaC** starting point lives in the **xwauth-api** repo:

- **[`deploy/terraform/stub/`](../../xwauth-api/deploy/terraform/stub/)** — creates a namespace, a generated **JWT** `Secret`, and installs [`deploy/helm/xwauth-api`](../../xwauth-api/deploy/helm/xwauth-api/). See that folder’s `README.md` for `terraform init` / `apply`.
