...
**Purpose:** Map **Keycloak**, **Auth0**, and **Supabase-shaped** sources into the **XW authorization server** client registry shape used by `xwauth` / `xwauth-connector-api` (env `XWAUTH_API_REGISTERED_CLIENTS_JSON`, file mount, or in-process `registered_clients`).  
