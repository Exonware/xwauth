# Architecture Reference — xwauth (XWOAuth)
...
**XWOAuth (xwauth)** is a **centralized, universal authorization system** in three parts: (1) **Backend library** (exonware-xwauth) for OAuth 1, OAuth 2, OIDC, SAML, future standards (e.g. DCE), tokens, sessions, **100+ connectors**, RBAC/ABAC/ReBAC/FGA; (2) **Authorization system with APIs** — OAuth/OIDC endpoints, user/session APIs, discovery; (3) **User interfaces** for login and admin flows. The library follows contracts/base/facade and provider abstraction. **Performance:** Use everything available in Exonware libraries for highest efficiency; extremely fast, really small footprint, really powerful.

---

## Boundaries

- **Public API:** Auth facades (XWAuth), token/session APIs, provider adapters, connector registry (target **≥100 connectors**).
...
- **Providers/connectors:** Pluggable implementations; consistent interface; **≥100 connectors** (current ~64); social, enterprise, gaming, AI, payments, etc.
- **Sessions / tokens:** Encrypted storage, lifecycle, validation; JWT and opaque; introspection (RFC 7662), revocation (RFC 7009).
- **Integration:** xwentity (users/roles), xwstorage (persistence), xwaction (workflow/security), xwbase; all future Exonware APIs. **Backward:** Extend auth provider for legacy systems.
...
5. **Providers:** Per-provider adapters (OAuth1/OAuth2/OIDC/SAML); registry; **≥100 connectors**.
...
- **xwstorage:** User/session persistence (optional; interface-based to avoid circular deps).
...
The **xwauth-server** stub exposes OAuth 2.0/OIDC (and OAuth1/SAML where applicable) endpoints and delegates to exonware-xwauth. Scope: authorization server, token endpoint, userinfo, discovery; see xwauth-server/docs/REF_22_PROJECT.md.
