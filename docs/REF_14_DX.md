# Developer Experience Reference — xwauth (XWOAuth) (REF_14_DX)

**Library:** exonware-xwauth  
**Last Updated:** 08-Feb-2026  
**Requirements source:** [REF_01_REQ.md](REF_01_REQ.md) sec. 5–6, sec. 8  
...
DX contract for xwauth (XWOAuth): **extremely short and seamless** code for authentication and authorization; **really small** surface, **really powerful**, **highest efficiency**. Filled from REF_01_REQ. Developer experience is a **top priority** — developers authenticate or use the code in the smallest, most efficient way.
...
| Task | Code |
|------|------|
| Configure and get facade | `from exonware.xwauth import XWAuth, XWAuthConfig`; `auth = XWAuth(config=XWAuthConfig(jwt_secret="secret"))` |
...
| Use a provider (e.g. Google) | Configure provider; use authorize flow — minimal code for sign-in. |

---

## Developer persona (from REF_01_REQ sec. 5)

Developer who needs **auth with minimal code**: **extremely short and seamless** — authenticate or use the code in the smallest, most efficient way; small surface, high power, highest efficiency. Primary users: eXonware developers and apps; xwstorage, xwbase, future APIs; developers integrating with legacy or external systems.
...
- **Facade:** `XWAuth`, `XWAuthConfig` — main API; authorize, token, introspect_token, revoke_token; device authorization (RFC 8628).
...
- **Providers:** Registry and pluggable adapters; **≥100 connectors** (current ~64).
...
1. **Integrate auth in minimal code:** Developer adds xwauth and gets sign-in in 1–3 lines.
...
5. **Use across Exonware:** xwstorage, xwbase, future APIs use xwauth as the single auth system.
