...
- OAuth/OIDC matrix and negative-path integration suites run in `xwauth-connector-api/tests/2.integration/`:
...
- Live discovery parity (issuer / `jwks_uri` across AS + OIDC docs), JWKS envelope, `jwks_uri` path fetch parity, RSA JWK field checks, active/next dedup: `xwauth-connector-api/tests/2.integration/test_oauth_endpoint_matrix.py`
- Token endpoint client auth interop (Basic vs POST client credentials): `xwauth-connector-api/tests/2.integration/test_oauth_interop_matrix.py`
- Introspect missing `token`, revoke client-auth failures: `xwauth-connector-api/tests/2.integration/test_oauth_oidc_negative_conformance.py`
...
- SCIM filter/patch interop edge cases: `xwauth/tests/1.unit/scim_tests/test_scim_filtering.py`, `xwauth/tests/1.unit/scim_tests/test_scim_patch.py`; list Users invalid `filter` / invalid `startIndex` / negative `count` (400): `xwauth-connector-api/tests/2.integration/test_scim_policy_api.py`
...
- Workflow: `xwauth-connector-api/.github/workflows/protocol-conformance.yml`
