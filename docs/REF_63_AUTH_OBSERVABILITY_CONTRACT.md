...
**Purpose:** Single entry point for **logs, traces, metrics, and audit** expectations across **xwauth-api** (transport) and **xwauth** (connector).  
...
The **xwauth-api** reference server attaches stable identifiers for cross-service joins (see package README):
...
- When OpenTelemetry is active, set **`XWAUTH_OPS_OTEL=true`** on the reference server (see xwauth-api README) so spans receive `xwauth.*` attributes where implemented.  
