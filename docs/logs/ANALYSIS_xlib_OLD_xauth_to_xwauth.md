# Analysis: xlib_OLD/xauth â†’ xwauth / xwauth-server
...
| `errors.py` | Hierarchical exceptions: xAuthError â†’ Config, Validation, Authentication, Token, RateLimit, Security/CSRF, MFA, OAuth, WebAuthn |
...
- Thin runner: builds FastAPI app, creates `XWAuth` (with optional xwstorage.connect), calls `register_auth_routes_from_services(app, auth, ...)`. All endpoint definitions stay in xwauth (single source of truth). Compliant with GUIDE_31 and architecture.
...
| **QA** | â€” | REF_50_QA gates | Use GUIDE_50 for gates; new code should not lower coverage on critical paths |
