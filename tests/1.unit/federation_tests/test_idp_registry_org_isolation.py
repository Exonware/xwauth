...
from exonware.xwauth.federation.idp_registry import TenantIdpRecord, TenantScopedIdpRegistry, idp_isolation_namespace
...
def test_resolve_domain_for_org_does_not_leak_across_orgs() -> None:
    auth = XWAuth(jwt_secret="idp-registry-test-secret")
...
async def test_discover_sso_org_scoped_without_legacy_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    auth = XWAuth(jwt_secret="saml-discovery-test-secret")
