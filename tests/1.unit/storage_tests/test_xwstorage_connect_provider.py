...
from exonware.xwauth.identity.storage.mock import MockToken, MockUser
from exonware.xwauth.identity.storage.xwstorage_connect_provider import (
    XWStorageConnectProvider,
)
...
@pytest.mark.asyncio
async def test_xwstorage_connect_provider_persists_user_across_instances() -> None:
    backend = _InMemoryBackend()
    provider1 = XWStorageConnectProvider(backend)
    await provider1.save_user(MockUser(id="u1", email="u1@example.com"))

    provider2 = XWStorageConnectProvider(backend)
...
@pytest.mark.xwauth_unit
@pytest.mark.asyncio
async def test_xwstorage_connect_provider_persists_token_lookup_indexes() -> None:
    backend = _InMemoryBackend()
    provider = XWStorageConnectProvider(backend)
...
@pytest.mark.xwauth_unit
@pytest.mark.asyncio
async def test_xwstorage_connect_provider_supports_basic_provider_storage_contract() -> None:
    backend = _InMemoryBackend()
    provider = XWStorageConnectProvider(backend)
