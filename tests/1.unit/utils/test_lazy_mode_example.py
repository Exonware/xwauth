# exonware/xwauth/tests/1.unit/utils/test_lazy_mode_example.py
...
def test_config_package_lazy_install_enabled_smoke() -> None:
    code = (
        "from exonware.xwlazy import config_package_lazy_install_enabled\n"
        "config_package_lazy_install_enabled('exonware.xwauth', enabled=True, mode='smart')\n"
    )
    proc = _run_snippet(code)
    if proc.returncode == 0:
        return
...
def test_config_package_lazy_install_enabled_disabled_noop() -> None:
    code = (
        "from exonware.xwlazy import config_package_lazy_install_enabled\n"
        "config_package_lazy_install_enabled('exonware.xwauth', enabled=False, mode='smart')\n"
    )
    proc = _run_snippet(code)
    if proc.returncode == 0:
        return
