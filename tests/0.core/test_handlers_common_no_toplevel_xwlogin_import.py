...
# exonware/xwauth.connector/tests/0.core/test_handlers_common_no_toplevel_xwlogin_import.py

"""Connector import hygiene: no module-level login-connector imports in xwauth src; lazy factory smoke (GUIDE_32/51)."""
...
def _module_level_xwlogin_imports(py_path: Path) -> list[tuple[int, str]]:
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.ImportFrom) and node.module:
            ...
            if mod == "exonware.xwauth.identity" or mod.startswith("exonware.xwauth.identity."):
            ...
            for alias in node.names:
                ...
                if name == "exonware.xwauth.identity" or name.startswith("exonware.xwauth.identity."):
...
def test_handlers_common_lazy_authenticator_factories_resolve_with_xwlogin() -> None:
    ...
    """Monorepo / [xwlogin] env: first call loads the login connector and returns real authenticator types (GUIDE_51)."""

    from types import SimpleNamespace



    from exonware.xwauth.identity.handlers import _common as handler_common
...
def test_xwauth_src_has_no_module_level_exonware_xwlogin_import() -> None:
    ...
    """Connector package must not bind the login connector at import time anywhere under ``src`` (GUIDE_32)."""
    ...
    assert not failures, (
        ...
        "exonware.xwauth.connector src must not import exonware.xwauth.identity at module level "
