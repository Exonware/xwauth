# exonware/xwauth.identity/providers/callback_providers.py
"""
...
def _get_provider_class(module_name: str) -> type | None:
    ...
    if module_name in _SKIP_MODULES:
        return None
    try:
        mod = importlib.import_module(f"exonware.xwauth.connect.providers.{module_name}")
    except Exception:
        return None
...
def discover_oauth2_callback_provider_names() -> list[str]:
    ...
    **provider name** used for routes is each provider's actual `provider_name`
    ...
    """
    from exonware.xwauth.connect import providers as _login_pkg
    names: set[str] = set()
